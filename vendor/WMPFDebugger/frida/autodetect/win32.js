    ...
    for (let index = 0; index < instructions.length; index++) {
        ...
        for (
            ...
            if (candidate.mnemonic === "mov" && secondMatch !== null) {
                // Keep the last assignment before the scene call. New WMPF
                // versions first load a scene object for validation and later
                // overwrite RCX with the actual argument passed to the call.
                secondOffset = Number.parseInt(secondMatch[1], 16);
            }

            const callee = directCallTarget(module, candidate);
            if (
                secondOffset !== null &&
                callee !== null &&
                hasSceneComparison(module, callee)
            ) {
...
const detect = async () => {
    ...
    // Newer WMPF builds can contain multiple wrappers referencing the
    // SendToClientFilter string. They are equivalent when their first direct
    // call resolves to the same filter implementation, so de-duplicate by the
    // callee rather than requiring a single wrapper.
    const cdpFilterOffsets = new Set(
        Array.from(xrefs.cdp).map((offset) => {
            const parent = findFunction(functions, offset);
            if (parent === null) {
                throw new Error(
                    `unable to resolve SendToClientFilter wrapper ${toHex(offset)}`,
                );
            }
            return findFirstCall(module, parent);
        }),
    );
    const cdpFilterOffset = requireUnique(
        "SendToClientFilter implementation",
        cdpFilterOffsets,
    );

    return {
        LoadStartHookOffset: toHex(loadStart.begin),
        CDPFilterHookOffset: toHex(cdpFilterOffset),
